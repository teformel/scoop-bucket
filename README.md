...
*   **HEU KMS Activator (heu-kms-activator)**：经典的 Windows/Office 全能本地及数字许可证激活工具。

### 本地私有/官方动态软件 (预留占位符清单)
*   **APK助手 (apk-helper)**：方便的本地 APK 快速安装器 (主页已关联 GitHub 源码库)。
*   **Alice in Cradle (alice-in-cradle)**：同人 2D 动作游戏 (已填入正版游戏最新客户端直链接口)。
*   **PCL 启动器 (pcl2)**：国内备受欢迎的《我的世界》第三方游戏启动器 (已填入云盘托管直链)。
*   **MyDockFinder (mydockfinder)**：仿 Mac 桌面美化工具 (已填入云盘托管直链)。
*   **搞机工具箱 (gj-toolbox)**：晨钟酱开发的多功能安卓玩机与激活调试工具 (已填入云盘托管直链)。
*   **几何画板 (geometers-sketchpad)**：经典的函数几何作图与教学辅助软件 (已填入云盘托管直链)。
*   **手写模拟器 (handwrite-simulator)**：仿人手写字迹排版生成器 (已填入云盘托管直链)。
*   **软媒魔方 (ruanmei-pcmaster)**：经典的 Windows 系统优化与垃圾清理工具箱去广告绿色便携版 (已填入云盘托管直链)。
